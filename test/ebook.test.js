// 53 kasus dari ebook "Dari Bingung Jadi Paham Waris" Bab 5–14 (port dari purwarupa/test.js).
// Wajib selalu lulus semua. Setiap perbaikan bug harus menambah kasus baru di test/regresi.test.js.
import { describe, it, expect } from "vitest";
import { hitung, fstr } from "../src/engine/index.js";

function T(name, pewaris, heirs, expected, extra = {}) {
  it(name, () => {
    const r = hitung({ pewaris, heirs });
    const got = {};
    r.rows.forEach(x => { got[x.key] = x.blocked ? "X" : fstr(x.each); });
    for (const [k, v] of Object.entries(expected)) expect(got[k], k).toBe(v);
    if (extra.am) expect(r.am, "asal masalah").toBe(extra.am);
    if (extra.aul) expect(r.amAul, "aul").toBe(extra.aul);
    if (extra.denom) expect(r.denom, "tashih").toBe(extra.denom);
    // bagian semua penerima harus berjumlah 1 (tidak ada harta hilang/berlebih)
    if (!r.sisa.n) expect(fstr(r.total)).toBe("1");
  });
}

describe("kasus ebook", () => {
